...
public class CArray<Hind extends Comparable<Hind>> {
    ...
    public Hind deleteLast(int L){
        Hind t = null;
        int index=L;
        if(!isEmpty(L)) {
            while (!isNull(index) && !isLast(index)) {
                index = cursorArray[index].getNext();
                t = cursorArray[index].getData();
            }
            cursorArray[index].setNext(cursorArray[index].getNext());
            free(index);
            return t;
        }
        return null;
    ...
    public Hind seeLast(int L){ // See Last is Not Working
        Hind t = null;
        int index=L;
        if(!isEmpty(L)) {
            while (!isNull(index) && !isLast(index)) {
